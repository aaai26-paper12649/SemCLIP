...
def evaluate_zeroshot(
    ...
    """
    Evaluate open_clip model on a classification dataset using zero-shot classification with standard and negated prompts.
    ...
    Args:
        ...
        dataset_name: Name of the dataset to evaluate on. Supported: 'cifar10', 'cifar100', 'caltech101',
                     'flowers102', 'food101', 'oxford_iiit_pet'
    ...
    Returns:
        dict: Dictionary with keys:
            - standard_accuracy: Accuracy using standard prompts (%)
            - negated_accuracy: Accuracy using negated prompts (%)
            - delta: Difference between standard and negated accuracy (%)
    """
    ...
    standard_prompts = [DEFAULT_PROMPT_TEMPLATE.format(cls) for cls in classes]
    negated_prompts = [DEFAULT_NEGATED_PROMPT_TEMPLATE.format(cls) for cls in classes]

    with torch.no_grad():
        standard_tokens = tokenizer(standard_prompts).to(device)
        negated_tokens = tokenizer(negated_prompts).to(device)

        standard_text_features = model.encode_text(standard_tokens)
        negated_text_features = model.encode_text(negated_tokens)

        standard_text_features = F.normalize(standard_text_features, dim=-1)
    ...
    with torch.no_grad():
        standard_features = model.encode_text(standard_tokens)
        standard_features = F.normalize(standard_features, dim=-1)
        ...
        for images, labels in tqdm(
            ...
            standard_logits = (image_features @ standard_text_features.t()).softmax(
                dim=-1
            )
            ...
            standard_preds = standard_logits.argmax(dim=-1)
            negated_preds = negated_logits.argmax(dim=-1)

            correct_standard += (standard_preds == labels).sum().item()
            correct_negated += (negated_preds == labels).sum().item()
            total += labels.size(0)

    standard_accuracy = 100.0 * correct_standard / total
    negated_accuracy = 100.0 * correct_negated / total
    delta = standard_accuracy - negated_accuracy

    print(f"{dataset_name.upper()} Zero-shot Results:")
    print(f"  Standard prompt accuracy: {standard_accuracy:.2f}%")
...
def evaluate_zeroshot_on_all_datasets(
    ...
    for dataset_name, dataset_class in dataset_configs:
        ...
        try:
            ...
            if "standard_accuracy" in metrics and "negated_accuracy" in metrics:
    ...
    if successful_evaluations:
        ...
        if num_successful > 0:
            avg_standard = total_standard_acc / num_successful
            avg_negated = total_negated_acc / num_successful
            avg_delta = avg_standard - avg_negated

            print("\n" + "=" * 60)
            print(f"AVERAGE ACROSS {num_successful} DATASETS:")
            print("=" * 60)
            print(f"  Average Standard Accuracy: {avg_standard:.2f}%")
